...
#error Unsupported platform. Open an issue and Ill probably add support.
        ...
        struct safe_module_enumerator {
            ...
            {
                value = valuengine->load_order_next();

                return value != head && valuengine->DllBase;
            }
        };
        ...
        struct lazy_module : lazy_base<lazy_module<OHP>> {
            ...
            LAZY_IMPORTER_FORCEINLINE static T get() noexcept
            {
                Enum e;
                do {
                    if (hash(e.valuengine->BaseDllName, get_offset(OHP)) == get_hash(OHP))
                        return (T)(e.valuengine->DllBase);
                } while (e.next());
                return {};
            }
        ...
        struct lazy_function : lazy_base<lazy_function<OHP, T>, T> {
            ...
            LAZY_IMPORTER_FORCEINLINE static F get() noexcept
            {
                ...
                do {
                    ...
                    if (!e.valuengine->DllBase || !e.valuengine->FullDllName.Length)
                        continue;
#endif

                    const exports_directory exports(e.valuengine->DllBase);
